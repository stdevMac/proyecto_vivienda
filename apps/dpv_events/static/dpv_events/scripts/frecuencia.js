var FrecuenciaScript = function () {
    ...
    frecuencia = function () {
        ...
        function initInput(){
            $("#id_name_frecuencia").val("").closest(".form-group").removeClass("has-success").closest(".form-group").removeClass("has-error");
            $("#id_name_frecuencia-error").remove();
            $("#id_name_frecuencia").maxlength({limitReachedClass: "label label-danger",threshold: 5});
            $("#id_days_frecuencia").val("").closest(".form-group").removeClass("has-success").closest(".form-group").removeClass("has-error");
            $("#id_days_frecuencia-error").remove();
        ...
        function editRow(oTable, nRow) {
            ...
            jqTds[2].innerHTML = '<input type="text" class="form-control input-large" value="' + aData[2] + '" required="">';
            jqTds[3].innerHTML = '' + 
        ...
        function addRow(oTable, nRow) {
            ...
            '<a class="edit btn default btn-xs purple" href="javascript:;"> Editar</a> ' +
                ...
                '<p class="text-center">' +
                'Seguro desea borrar el Frecuencia' +
                '</p>' +
                '</div>' +
        ...
        function saveRow(oTable, nRow) {
            ...
            $.post('/dpv_events/update_frecuencia/', {
                'id': aData[0],
                'name_frecuencia': jqInputs[0].value,
                'days_frecuencia': jqInputs[1].value,
                
            }, function (data) {
                oTable.fnUpdate([data.id, data.name,data.days, '<a class="edit btn default btn-xs purple" href="javascript:;"> Editar</a> ' +
        ...
        form.validate({
            ...
            submitHandler: function (form) {
                ...
                var formData = new FormData($("#newfrecuencia_form")[0]);
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (data.isfirst) {
                            $('#cancelar').click();
                            alert_success('Se Ha Agregado Un Nuevo Frecuencia.');
                            setTimeout(function() {
                                $(location).attr('href',"");
                            }, 1250);
                            return;
                        ...
                        var aiNew = oTable.fnAddData([data.id, data.name,data.days, '']);
        ...
        table.on('click', '.delete', function (e) {
            ...
            var aData = oTable.fnGetData(nRow);
            $.post('/dpv_events/delete_frecuencia/', {
                'id': aData[0]
            }, function (data) {
                if (data.iserror) {
                ...
                if(count == 0){
                    $('#frecuencia-body').html('<div class="no-result theme-font"> No hay Frecuencias </div>');
                }
